...
The mean and median of the total number of steps taken per day are `r totalMean` and `r totalMedian` respectively.
...
The 5-minute interval with the highest average of steps accross all days was `r maxAvg` at `r paste0(substr(names(posMaxAvg),1,1), ":", substr(names(posMaxAvg),2,3))` am.
...
There are `r totalMissingValues` missing values (number of steps column) in the data.
...
We can appreciate that there are no NAs in our new dataframe filledData: `r sum(is.na(filledData))`.
Performing the same analysis with the complemented data we have:

```{r calculations no NAs}
